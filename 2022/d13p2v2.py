...
divider_packets = [[[2]], [[6]]]
divider_packets.sort(key=functools.cmp_to_key(compare))

print(
    math.prod(
        [
            divider_packet_index
            + 1
            + [compare(line, divider_packet) for line in packets].count(-1)
            for divider_packet_index, divider_packet in enumerate(divider_packets)
        ]
    )
)
