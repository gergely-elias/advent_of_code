...
for iteration in range(2):
    for y in range(target_y + beyond_target + 1):
        x_min = (
            target_x + previous_beyond_target + 1
            if y < target_y + previous_beyond_target
            else 0
        )
        for x in range(x_min, target_x + beyond_target + 1):
            ...
            erosion_level[y, x] = (geologic_index[y, x] + cave_depth) % cave_generator[
                2
            ]
            ...
            distance.add_edge(
                tuple(current_region + [possible_equipment[0]]),
                tuple(current_region + [possible_equipment[1]]),
                weight=equipment_switch_time,
            )
            for coord in range(len(current_region)):
                if current_region[coord] > 0:
                    ...
                    for equipment in matching_equipment:
                        distance.add_edge(
                            tuple(current_region + [equipment]),
                            tuple(neighbour_region + [equipment]),
                            weight=move_time,
                        )

    shortest_path_length = networkx.dijkstra_path_length(
        distance,
        source=(source_y, source_x, 1),
