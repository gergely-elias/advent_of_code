...
while True:
    ...
    if landscape in landscape_history:
        ...
        matching_landscape = landscape_history[
            target_minute
            - 1
            - (target_minute - 1 - landscape_history.index(landscape))
            // period
            * period
        ]
        print(
            list(matching_landscape.values()).count(1)
            * list(matching_landscape.values()).count(2)
        )
        break
    else:
