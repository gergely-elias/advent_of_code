...
while not combat_over:
    ...
    for targetor_group in all_groups:
        ...
        attackable_opponents = [
            ...
            if group["class"] != targetor_group["class"]
            and not (group["init"] in target.values())
        ]
        for defender_group in attackable_opponents:
            defender_group["damage_factor"] = calculate_damage_factor(
                defender_group, attack_type
            )
        attackable_opponents.sort(
            key=lambda opponent_lookup: (
                -opponent_lookup["damage_factor"],
                -opponent_lookup["power"],
                -opponent_lookup["init"],
            )
        )
        if (
            len(attackable_opponents) > 0
            and attackable_opponents[0]["damage_factor"] > 0
        ):
    ...
    for attacker_group in all_groups:
        if attacker_group["init"] in target:
            opponent_lookup = find_group_with_initiative(
                all_groups, target[attacker_group["init"]]
            )
            if opponent_lookup[0] > -1:
                ...
                defender_group["units"] -= (
                    defender_group["damage_factor"]
                    * attacker_group["power"]
                    // defender_group["hp"]
                )
                defender_group["power"] = (
                    defender_group["units"] * defender_group["damage"]
                )
                if defender_group["units"] <= 0:
                    opponents = [immune_system, infections][defender_group["class"]]
                    del opponents[
                        find_group_with_initiative(opponents, defender_group["init"])[0]
                    ]
