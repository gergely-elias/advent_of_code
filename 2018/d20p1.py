...
def preprocess_first_parenthesis(route_regexp, nodes):
  ...
  if first_parenthesis_start > -1:
    ...
    next_nodes = split_on_the_highest_level(route_regexp[first_parenthesis_start + 1 : first_parenthesis_end], next_nodes)
    return preprocess_first_parenthesis(route_regexp[first_parenthesis_end + 1 :], next_nodes)
  else:
...
print max(networkx.shortest_path_length(G, source = start_node).values())
