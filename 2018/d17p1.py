...
def flow(source_y,source_x):
  ...
  elif water_map[(source_y + 1, source_x)] == '#' or water_map[(source_y + 1, source_x)] == '~':

    next_wall_right = None
    sink_or_wall_lookup = source_x + 1
    while (water_map[(source_y + 1, sink_or_wall_lookup)] == '#' or water_map[(source_y + 1, sink_or_wall_lookup)] == '~') and (water_map[(source_y, sink_or_wall_lookup)] == '.' or water_map[(source_y, sink_or_wall_lookup)] == '|'):
...
print len([v for (y, x), v in water_map.items() if (y in range(min_y, max_y + 1) and v in ['~', '|'])])
