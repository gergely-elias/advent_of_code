...
while num_of_elves > 0 and num_of_goblins > 0:
    ...
    while creature_index < len(creatures):
        ...
        if minimal_open_square_distance > 0 and minimal_open_square_distance < float(
            "inf"
        ):
            ...
            while decreasing_distance_loop > 0:
                ...
                for direction in directions:
                    ...
                    if (
                        distances[possible_next_on_path[0]][possible_next_on_path[1]]
        ...
        attackable_opponents = list(
            filter(
                lambda opponent: sum(
                    [
                        abs(coord1 - coord2)
                        for coord1, coord2 in zip(opponent[:2], creature_on_turn[:2])
                    ]
                )
                == 1,
                all_opponents,
        ...
        if len(attackable_opponents):
            ...
            weakest_attackable_opponents = list(
                filter(lambda opponent: opponent[3] == minimal_hp, attackable_opponents)
            )
            ...
            if reduced_health_points > 0:
                ...
                if opponents_creature_index < creature_index:
                    creature_index -= 1
                if creature_index < len(creatures) - 1 and (
                    num_of_elves == 0 or num_of_goblins == 0
                ):
