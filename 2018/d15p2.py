...
def simulate_battle(elf_hit_points):
  ...
  while num_of_goblins > 0:
    ...
    while creature_index < len(creatures):
      ...
      distances = [[float("inf")] * len(world_map[map_row_index]) for map_row_index in range(len(world_map))]
      breadth_first_search(tuple(creature_on_turn[:2]))

      open_square_distances = [distances[square[0]][square[1]] for square in open_squares]
      minimal_open_square_distance = min(open_square_distances + [float("inf")])
      if minimal_open_square_distance > 0 and minimal_open_square_distance < float("inf"):
        move_target_square = open_squares[open_square_distances.index(minimal_open_square_distance)]
        ...
        while decreasing_distance_loop > 0:
          ...
          for direction in directions:
            ...
            if distances[possible_next_on_path[0]][possible_next_on_path[1]] == decreasing_distance_loop:
              ...
              break
        world_map[backward_path_from_target[-1][0]][backward_path_from_target[-1][1]] = '.'
        world_map[backward_path_from_target[-2][0]][backward_path_from_target[-2][1]] = creatures[creature_index][2]
      ...
      if len(attackable_opponents):
        ...
        if reduced_health_points > 0:
          creatures[opponents_creature_index] = (opponent_to_attack[0], opponent_to_attack[1], opponent_to_attack[2], reduced_health_points)
        else:
...
elf_hit_point_thresholds = sorted(list(filter(lambda hit_point: hit_point > 3, set([(health_points_at_start - 1) / num_of_hits_to_kill + 1 for num_of_hits_to_kill in range(1, health_points_at_start + 1)]))))
