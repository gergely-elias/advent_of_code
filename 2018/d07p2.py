...
while True:
    just_finished_tasks = list(
        filter(lambda task: tasks_in_progress[task] == time, tasks_in_progress)
    )
    ...
    startable_tasks = sorted(
        list(
            tasks.difference(tasks_with_prerequirement).difference(
