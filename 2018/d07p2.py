...
while True:
  just_finished_tasks = filter(lambda task: tasks_in_progress[task] == time, tasks_in_progress)
  ...
  dependencies = filter(lambda x: x[0] not in just_finished_tasks, dependencies)
