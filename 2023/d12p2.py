...
for condition_record in [line.strip() for line in input_lines]:
    folded_springs, folded_damaged_groups = condition_record.split()
    fold_copies = 5
    springs = "?".join([folded_springs] * fold_copies)
    damaged_groups = tuple(map(int, folded_damaged_groups.split(","))) * fold_copies
