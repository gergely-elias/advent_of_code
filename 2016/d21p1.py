...
operations = [line.strip() for line in input_lines]
password = "abcdefgh"
password_length = len(password)
index_based_rotation = dict()
for index in range(password_length):
    index_based_rotation[index] = (
...
for operation in operations:
    if operation.startswith("swap"):
        ...
        if operation.startswith("rotate left"):
            ...
            rotation = index_based_rotation[letter_index]
        ...
        password = password[:pos_x] + password[pos_x + 1 :]
        password = password[:pos_y] + letter + password[pos_y:]
print(password)
