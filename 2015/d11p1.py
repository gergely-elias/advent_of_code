...
def jump_to_valid_word(converted_password):
    for index, charcode in enumerate(converted_password):
        if charcode in forbidden_charcodes:
            return step(converted_password, index)[: index + 1] + [
                smallest_valid_charcode
            ] * (len(converted_password) - index - 1)
    return step(converted_password, -1)


def is_valid_word(converted_password):
    charcode_diffs = [
        j - i for i, j in zip(converted_password[:-1], converted_password[1:])
    ]
    pair_positions = [i for i, x in enumerate(charcode_diffs) if x == 0]
    if len(pair_positions) > 2 or (
        len(pair_positions) == 2 and pair_positions[1] - pair_positions[0] != 1
    ):
        straight_positions = [i for i, x in enumerate(charcode_diffs) if x == 1]
        straight_position_diffs = [
            l - k for k, l in zip(straight_positions[:-1], straight_positions[1:])
        ]
...
while not is_valid_word(converted_password):
