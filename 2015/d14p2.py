...
for line in input_lines:
  ...
  dist = [(t // period * nums[1] + min(t % period, nums[1])) * nums[0] for t in range(1, time + 1)]
...
for t in range(time):
  ...
  points = [points[reindeer] + (1 if (positions[reindeer] == max(positions)) else 0) for reindeer in range(len(dist_over_time))]
print(max(points))
