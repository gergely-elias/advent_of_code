    ...
    for old_submolecule, new_submolecule in replacements:
        if old_submolecule == "e":
            ...
            if len(new_submolecule) > 2:
                if (
                    not (
                        len(new_submolecule) % 2 != 0
                        or any(
                            [
                                new_submolecule[i] in replacable_molecules
                                for i in range(1, len(new_submolecule), 2)
                            ]
                        )
                    )
                    ...
                    and (
                        ...
                        or all(
                            [
                                ...
                                for i in range(3, len(new_submolecule) - 3, 2)
                            ]
                        )
                    )
                ):
                    ...
                    regex_separator = (
                        new_submolecule[3]
                        if len(new_submolecule) > 4
                        else regex_separator
                    )
                    replacement_classes[1].add(
                        len([atom in replacable_molecules for atom in new_submolecule])
                    )
                else:
                    exit()
            else:
...
if len(molecule_length_changes[0]) == 1 and min(molecule_length_changes[1]) == 2:
    print(
        ...
        - 2
        * (
            original_molecule.count(regex_start)
            + original_molecule.count(regex_separator)
        )
        - (list(molecule_length_changes[0])[0] - 1)
    )
