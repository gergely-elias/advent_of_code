...
for sitting_order in itertools.permutations(people):
  current_happiness = 0
  for seat_index in list(range(len(sitting_order))):
    current_happiness += happiness_relations[(sitting_order[seat_index], sitting_order[(seat_index + 1) % len(people)])] + happiness_relations[(sitting_order[(seat_index + 1) % len(people)], sitting_order[seat_index])]
