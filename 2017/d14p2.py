...
binary_representation = [
    [
        int(digit)
        for digit in bin(int(knot_hash(word + "-" + str(row)), 16))[2:].zfill(size)
    ]
    ...
    for j in range(size):
        if binary_representation[i][j] == 1:
            ...
            for offset in neighbour_offsets:
                if (
                    i + offset[0] in range(size)
                    and j + offset[1] in range(size)
                    and binary_representation[i + offset[0]][j + offset[1]] == 1
                ):
