...
for particle1, particle2 in itertools.combinations(particles, 2):
  ...
  for coordinate_index in range(3):
    delta_acceleration = particle1['acceleration'][coordinate_index] - particle2['acceleration'][coordinate_index]
    delta_velocity = particle1['velocity'][coordinate_index] - particle2['velocity'][coordinate_index]
    delta_position = particle1['position'][coordinate_index] - particle2['position'][coordinate_index]
    roots = numpy.roots([delta_acceleration, delta_acceleration + 2 * delta_velocity, 2 * delta_position])
    nonnegative_roots_rounded = set(filter(lambda root: root >= 0, [int(numpy.real(root)) for root in roots]))
