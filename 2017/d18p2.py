...
while not (
    program_state[0]["waiting_for_receive"] and program_state[1]["waiting_for_receive"]
):
    while (
        program_state[program_id]["instruction_index"] in range(0, len(instructions))
        and not program_state[program_id]["waiting_for_receive"]
    ):
        instruction = instructions[program_state[program_id]["instruction_index"]]
        if instruction[0] == "set":
            program_state[program_id]["registers"][instruction[1]] = substitute(
                program_id, instruction[2]
            )
            ...
            program_state[program_id]["instruction_index"] += substitute(
                program_id, instruction[2]
            )
            continue
        elif instruction[0] == "rcv":
            if len(program_state[program_id]["receive_queue"]) > 0:
                program_state[program_id]["registers"][instruction[1]] = program_state[
                    program_id
                ]["receive_queue"].pop(0)
            else:
            ...
            program_state[1 - program_id]["receive_queue"].append(
                substitute(program_id, instruction[1])
            )
