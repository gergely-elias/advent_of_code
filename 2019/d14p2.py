...
while step > 0:
  ...
  while set(ingredients.keys()) != set(['ORE']):
    ...
    for chemical in ingredients:
      if len(set([reaction[1][0] for reaction in dependencies.edges() if reaction[0][0] == chemical]).difference(processed)) == 0:
        processed_next.append(chemical)
        for output_chemical, output_amount in [reaction[1] for reaction in dependencies.edges()]:
          if output_chemical == chemical:
            reaction_amount = math.ceil(ingredients[chemical] / output_amount)
        for input_chemical, input_amount in [reaction[0] for reaction in dependencies.edges() if reaction[1][0] == chemical]:
