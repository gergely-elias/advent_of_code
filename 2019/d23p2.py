...
waiting_for_input = [False] * number_of_computers
nat_memory = []
last_y_sent_from_nat = None

computer = 0
while not all(computer_done):
  if all(waiting_for_input) and len(nat_memory) > 0:
    if nat_memory[-1] == last_y_sent_from_nat:
      print(nat_memory[-1])
      exit()
    else:
      inputs_computers[0].extend(nat_memory)
      waiting_for_input[0] = False
      last_y_sent_from_nat = nat_memory[-1]
