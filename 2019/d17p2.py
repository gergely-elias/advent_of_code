...
def compress(
    ...
    if len(compression_mappings) > 0:
        while any(
            [
                commands_complete_list[
                    compression_position : compression_position + len(mapping)
                ]
                == mapping
                for mapping in compression_mappings
            ]
        ):
            matched_mapping_index = [
                commands_complete_list[
                    compression_position : compression_position + len(mapping)
                ]
                == mapping
    ...
    if compression_position == len(commands_complete_list):
        if len(compression_mappings) <= 3 and all(
            [len(",".join(mapping)) <= 20 for mapping in compression_mappings]
        ):
            inputs_to_encode = (
                [",".join(["ABC"[i] for i in compressed_version]) + "\n"]
                + [",".join(mapping) + "\n" for mapping in compression_mappings]
                + ["n\n"]
            )
            inputs = [ord(character) for character in "".join(inputs_to_encode)]
    if len(compression_mappings) > 3 or any(
        [len(",".join(mapping)) > 20 for mapping in compression_mappings]
    ):
        return
    ...
    for next_length in range(1, len(commands_complete_list) - compression_position):
        compress(
            commands_complete_list,
            compressed_version + [len(compression_mappings)],
            compression_position + next_length,
            compression_mappings
            + [
                commands_complete_list[
                    compression_position : compression_position + next_length
                ]
            ],
        )
                        ...
                        tuple(
                            [
                                ...
                                for (current, step) in zip(
                                    robot_position, direction_steps[robot_direction]
                                )
                            ]
                        )
                    ]
                    ...
                    scaffold[
                        tuple(
                            [
                                ...
                                for (current, step) in zip(
                                    robot_position,
                                    direction_steps[(robot_direction + 1) % 4],
                                )
                            ]
                        )
                    ]
