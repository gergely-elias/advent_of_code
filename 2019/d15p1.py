...
while position < len(program):
  ...
  if operation == 1:
    ...
    new_position = tuple([current + step for (current, step) in zip(current_droid_position, direction_steps[current_direction])])
    if output_parameter == 0:
      ...
      for y in range(min([discovered_cell[0] for discovered_cell in droid_map.keys()]), max([discovered_cell[0] for discovered_cell in droid_map.keys()]) + 1):
        for x in range(min([discovered_cell[1] for discovered_cell in droid_map.keys()]), max([discovered_cell[1] for discovered_cell in droid_map.keys()]) + 1):
          current_cell = (y, x)
          if droid_map[current_cell] == '.':
            for lookaround_direction in direction_steps:
              lookaround_cell = tuple([current + step for (current, step) in zip(current_cell, lookaround_direction)])
      ...
      if not undiscovered_area:
        print(networkx.shortest_path_length(paths, droid_start_position, oxygen_system_location))
        exit()
  elif operation == 5:
    if address[0] != 0:
      position = address[1]
