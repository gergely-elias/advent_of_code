...
for row_index in range((len(input_lines))):
  for col_index in range(len(input_lines[0])):
    ...
    elif mazemap[(row_index, col_index)] >= 'a' and mazemap[(row_index, col_index)] <= 'z':
...
for row_index in range((len(input_lines))):
  for col_index in range(len(input_lines[0])):
    ...
    for direction in directions:
      neighbour_cell = tuple([cell_coord + dir_coord for cell_coord, dir_coord in zip(cell, direction)])
...
for door_key in door_keys:
  quadrant = '1234'[[networkx.has_path(mazegraph, positions[drone], positions[door_key]) for drone in '1234'].index(True)]
  prereq_keys = [door.lower() for door in (list(set([mazemap[cell] for cell in networkx.shortest_path(mazegraph, positions[quadrant], positions[door_key])]).intersection(doors)))]
...
while len(new_nodes) > 0:
  ...
  for current_node in recent_nodes:
    ...
    for letter in door_keys:
      quadrant_index = [networkx.has_path(mazegraph, positions[drone], positions[letter]) for drone in '1234'].index(True)
      quadrant = '1234'[quadrant_index]
      if letter not in collected_keys and len(set(networkx.ancestors(key_dependencies, letter)).difference(set(collected_keys))) == 0:
        updated_last_keys = list(last_keys[:])
        updated_last_keys[quadrant_index] = letter
        new_node = (''.join(sorted(collected_keys + letter)), ''.join(updated_last_keys))
        new_nodes.add(new_node)
        distances.add_edge(current_node, new_node, weight = networkx.shortest_path_length(mazegraph, positions[last_keys[quadrant_index]], positions[letter]))

min_distance = float('inf')
distance_from_source = networkx.single_source_dijkstra_path_length(distances, ('', '1234'))
