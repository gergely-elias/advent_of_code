...
for row_index in range((len(input_lines))):
  for col_index in range(len(input_lines[0])):
    if mazemap[(row_index, col_index)] >= 'a' and mazemap[(row_index, col_index)] <= 'z':
...
for door_key in door_keys:
  prereq_keys = [door.lower() for door in (list(set([mazemap[cell] for cell in networkx.shortest_path(mazegraph, positions['@'], positions[door_key])]).intersection(doors)))]
  ...
  for current_node in recent_nodes:
    ...
    for letter in door_keys:
      if letter not in collected_keys and len(set(networkx.ancestors(key_dependencies, letter)).difference(set(collected_keys))) == 0:
        ...
        distances.add_edge(current_node, new_node, weight = networkx.shortest_path_length(mazegraph, positions[last_key], positions[letter]))
