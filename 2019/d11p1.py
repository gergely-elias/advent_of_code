...
painted = collections.defaultdict(lambda: False)
next_step = 'paint'
...
while position < len(program):
  ...
  if operation == 1:
    ...
    if next_step == 'paint':
      ...
      painted[robot_position] = True
      next_step = 'move'
    else:
      robot_direction = (robot_direction + 2 * output_parameter - 1) % 4
      robot_position = tuple([current + move for (current, move) in zip(robot_position, direction_coords[robot_direction])])
      next_step = 'paint'
  elif operation == 5:
    if address[0] != 0:
      position = address[1]
