...
for line_index in range(len(input_lines)):
  ...
  for instruction in wire:
    ...
    next_pos[['y','x'][direction % 2]] = last_pos[['y','x'][direction % 2]] + distance * (1 - (direction // 2) * 2)
...
for wire0_part_index in range(len(wires[0]) - 1):
  ...
  for wire1_part_index in range(len(wires[1]) - 1):
    ...
    if intersection_x is not None and intersection_y is not None:
      current_distance = \
        abs(intersection_x - wire0_part[0]['x']) + \
        abs(intersection_y - wire0_part[0]['y']) + \
        wire0_part[0]['d'] + \
        abs(intersection_x - wire1_part[0]['x']) + \
        abs(intersection_y - wire1_part[0]['y']) + \
        wire1_part[0]['d']
