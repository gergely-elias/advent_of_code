...
for iteration in range(200):
  next_multilevel_grid = collections.defaultdict(lambda: collections.defaultdict(lambda: '.'))
  for grid_level in range(-iteration - 1, iteration + 2):
    ...
    for current in itertools.product(range(height), range(width)):
      ...
      for direction in directions:
        ...
        neighbour = tuple([cell + step for (cell, step) in zip(current, direction)])
        if multilevel_grid[grid_level][neighbour] == '#':
          neighbour_count += 1

        if neighbour[non_zero_coord_index] in [-1, size_in_direction]:
          neighbour_count += 1 if multilevel_grid[grid_level - 1][tuple([cell + step for (cell, step) in zip(center_coord, direction)])] == '#' else 0

        if current == tuple([cell + step for (cell, step) in zip(center_coord, direction)]):
          neighbour_cell = [None] * 2
          neighbour_cell[non_zero_coord_index] = size_in_direction - 1 if direction[non_zero_coord_index] == 1 else 0
          for coord_loop in range(size_in_normal_direction):
            neighbour_cell[zero_coord_index] = coord_loop
            neighbour_count += 1 if multilevel_grid[grid_level + 1][tuple(neighbour_cell)] == '#' else 0

      if multilevel_grid[grid_level][current] == '#' and neighbour_count == 1:
        next_grid[current] = '#'
      elif multilevel_grid[grid_level][current] == '.' and neighbour_count in [1, 2]:
        next_grid[current] = '#'
    next_multilevel_grid[grid_level] = next_grid
  multilevel_grid = copy.deepcopy(next_multilevel_grid)

bug_count = 0
for (level, line_index, col_index) in itertools.product(range(-iteration - 1, iteration + 2), range(height), range(width)):
