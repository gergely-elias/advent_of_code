...
for line_index in range(2, len(input_lines) - 2):
  breadth = min(breadth, min(list(map(len, re.findall('[\.#]+', maze_lines[line_index])))))
...
for y in range(donut_height_with_portals):
  for x in range(donut_width_with_portals):
    ...
    for step_dir in [(-1, 0), (0, 1), (1, 0), (0, -1)]:
      neighbour_coord = tuple([current + step for (current, step) in zip(coord, step_dir)])
      if mazemap[coord] not in ' #' and mazemap[neighbour_coord] not in ' #':
        maze.add_edge(coord, neighbour_coord)

donut_edges = [([1, donut_height + 2 - breadth - 1], range(2, donut_width + 2), (-1, 0)), \
  ...
  for idxy, y in enumerate(edge_y):
    for idxx, x in enumerate(edge_x):
      ...
      if mazemap[portal_name_coord] not in ' #.':
        portal_name = mazemap[tuple([coord + step for (coord, step) in zip(portal_name_coord, edge_normal_direction)])] + mazemap[(y,x)]
        if sum(edge_normal_direction) == 1:
          portal_name = ''.join(reversed(portal_name))
        if inner_ridge:
          portal_name += '+'
        portals[portal_name] = tuple([coord - step for (coord, step) in zip(portal_name_coord, edge_normal_direction)])
...
for portalpair in itertools.combinations(portals, 2):
  if networkx.has_path(maze, *[portals[portal] for portal in portalpair]):
    connected_portals[portalpair] = networkx.shortest_path_length(maze, *[portals[portal] for portal in portalpair])
...
while (2 * layer + 1) * (min(connected_portals.values()) + 1) - 1 < min_distance:
  for edge in connected_portals:
    multilayer_maze.add_edge(portal_with_layer(edge[0], layer), portal_with_layer(edge[1], layer), weight = connected_portals[edge] + 1)
  if networkx.has_path(multilayer_maze, 'AA0', 'ZZ0'):
    min_distance = networkx.shortest_path_length(multilayer_maze, 'AA0', 'ZZ0', weight='weight') - 1
  layer += 1
print(min_distance)
