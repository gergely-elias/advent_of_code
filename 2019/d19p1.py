...
while input_parameter_idx < len(inputs):
    program = collections.defaultdict(lambda: 0)
    for memory_index, code in enumerate(orig_program):
        program[memory_index] = code
    position = 0
    ...
    while position < len(program):
        ...
        if operation == 1:
            ...
            input_parameter = inputs[input_parameter_idx]
            program[write_parameter_address] = input_parameter
            input_parameter_idx += 1
        elif operation == 4:
            output_parameter = address[0]
            affected_points += output_parameter
        elif operation == 5:
            if address[0] != 0:
                position = address[1]
                continue
