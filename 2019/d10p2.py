    ...
    if asteroidmap[candidate_y][candidate_x] == '#':
      for other_y in range(height):
        for other_x in range(width):
          if asteroidmap[other_y][other_x] == '#' and (other_y != candidate_y or other_x != candidate_x):
            ...
            for step in range(1, divisor):
              if asteroidmap[candidate_y + step * distance_y_unit][candidate_x + step * distance_x_unit] == '#':
                visible = False
                break
            if visible:
              count += 1
...
for target_y in range(height):
  for target_x in range(width):
    if asteroidmap[target_y][target_x] == '#' and (target_y != laser_y or target_x != laser_x):
      targets.append({'direction': -math.atan2(target_x - laser_x, target_y - laser_y), 'distance': (target_y - laser_y) ** 2 + (target_x - laser_x) ** 2, 'x': target_x, 'y': target_y})
targets.sort(key = lambda target: (target['direction'], target['distance']))

sort_index = 0
while sort_index != len(targets) - 2:
  if len(set([targets[unsorted_target_index]['direction'] for unsorted_target_index in range(sort_index, len(targets))])) == 1:
    break
